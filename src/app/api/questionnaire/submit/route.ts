...
export async function POST(request: Request) {
  ...
  if (jsonBody.questionnaireVersion !== "v1") {
    return NextResponse.json({ error: "Unsupported questionnaire version." }, { status: 400 });
  }

  const localId = typeof jsonBody.localId === "string" ? jsonBody.localId.trim() : "";
  if (!localId) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Unexpected answer validation error." }, { status: 500 });
  }
  ...
  const updateBody = {
    fields: {
      initialFootprintKg: { integerValue: Math.round(footprint.totalKgPerYear).toString() }
    }
  };
